...
uint256 hashGenesisBlock("0x00000f25581e131256f42a26d4ca4e9d6ccfec505d02e48aa565b3d21ffd232a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // StarbucksCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "StarbucksCoin Signed Message:\n";
...
{
    // StarbucksCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight == 1)
    {
        nSubsidy = 653600 * COIN;
    }
	else if (nHeight <= 1440 && nHeight > 1)
    {
        nSubsidy =888 * COIN;
    }
	else if (nHeight <= 8640 && nHeight > 1440)
    {
        nSubsidy =748 * COIN;
    }
	else if (nHeight <= 17280 && nHeight > 8640)
    {
        nSubsidy =628 * COIN;
    }
	else if (nHeight <= 25920 && nHeight > 17280)
    {
        nSubsidy = 578 * COIN;
    }
	else if (nHeight <= 34560 && nHeight > 25920)
    {
        nSubsidy = 548 * COIN;
    }
	else if (nHeight <= 43200 && nHeight > 34560)
    {
        nSubsidy = 488 * COIN;
    }
...
static const int64 nTargetTimespan = 50 * 60; // StarbucksCoin: 
static const int64 nTargetSpacing = 60; // StarbucksCoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // StarbucksCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 60; // StarbucksCoin: 1 minute
        static const unsigned int TimeDaySeconds = 50 * 60; // StarbucksCoin: 1 day
...
{
    // StarbucksCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
		...
		const char* pszTimestamp = " 27 May 2014 the coin coming ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1401082761;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1241139553;
        ...
        assert(block.hashMerkleRoot == uint256("0xda1ef2d6fc8c6d6a056e1e09a2fdfab7903103cacd6258876535ae2d91dc6789"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // StarbucksCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StarbucksCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StarbucksCoinMiner : ProcessBlock, block not accepted");
    }
